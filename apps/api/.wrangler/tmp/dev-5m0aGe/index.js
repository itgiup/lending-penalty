...
// .wrangler/tmp/bundle-1YFHMv/checked-fetch.js
...
// ../../node_modules/.pnpm/@hono+zod-validator@0.2.2_hono@4.12.23_zod@3.25.76/node_modules/@hono/zod-validator/dist/esm/index.js
...
var app = new Hono2();
app.use("*", cors());
var generateId = /* @__PURE__ */ __name(() => Math.random().toString(36).substr(2, 9) + Date.now().toString(36), "generateId");
var calculatePeriods = /* @__PURE__ */ __name((startDate, endDate = /* @__PURE__ */ new Date()) => {
  const start = new Date(startDate);
  const end = new Date(endDate);
  const daysDiff = (end.getTime() - start.getTime()) / (1e3 * 60 * 60 * 24);
  return daysDiff / 365;
}, "calculatePeriods");
var calculateCompoundInterest = /* @__PURE__ */ __name((principal, annualRate, periods) => {
  const totalAmount = principal * Math.pow(1 + annualRate, periods);
  return totalAmount - principal;
}, "calculateCompoundInterest");
var calculateLoanStatus = /* @__PURE__ */ __name((loan, currentDate = /* @__PURE__ */ new Date()) => {
  const {
    principal,
    interestRate,
    penaltyRate,
...
var loanSchema = external_exports.object({
  borrower_name: external_exports.string().min(1, "Borrower name is required"),
  borrower_phone: external_exports.string().optional(),
  borrower_address: external_exports.string().optional(),
  principal: external_exports.number().positive("Principal must be positive"),
  interest_rate: external_exports.number().min(0).max(100),
  penalty_rate: external_exports.number().min(0).max(100),
  start_date: external_exports.string().datetime("Invalid date format"),
  term_months: external_exports.number().int().positive("Term must be positive"),
  notes: external_exports.string().optional()
});
var paymentSchema = external_exports.object({
  amount: external_exports.number().positive("Amount must be positive"),
  payment_date: external_exports.string().datetime("Invalid date format"),
  notes: external_exports.string().optional()
});
app.get("/health", (c) => {
...
// .wrangler/tmp/bundle-1YFHMv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1YFHMv/middleware-loader.entry.ts
